...
func init() {
	android.RegisterModuleType("orgasmic_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = orgasmicExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = orgasmicExpandVariables(ctx, dep_file)
	...
	cmd := orgasmicExpandVariables(ctx, String(g.properties.Cmd))
